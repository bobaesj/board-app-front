...
const Board = () => {
    ...
    const findById = useCallback(async () => {
        try {
            const response = await axios.get(`http://211.188.55.132:9090/boards/${id}`, {
                headers: {
    ...
    const deleteById = useCallback(async() => {
        try{
            const response = await axios.delete(`http://211.188.55.132:9090/boards/${id}`, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
                            ...
                            >
                                ...
                                <img width='150px'
                                     ...
                                     src={boardFile.filetype === 'image'
                                        ? `https://kr.object.ncloudstorage.com/bobaesj/${boardFile.filepath}${boardFile.filename}`
            ...
            <Grid container style={{marginTop: '3%', textAlign: 'center'}}>
                <Grid item
                      ...
                      style={
                        ...
                        <Button type='button' variant='contained' style={{marginLeft: '2%'}} onClick={deleteById}>삭제</Button>
                </Grid>
            </Grid>
        </form>
    </Container>
